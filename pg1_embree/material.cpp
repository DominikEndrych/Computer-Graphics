...
Material::Material( std::string & name, const Vector3 & ambient, const Vector3 & diffuse,
	...
	const float shininess, const float ior, Texture ** textures, const int no_textures, int shader )
{
	name_ = name;

	this->ambient = ConvertToLinear(ambient);
	this->diffuse = ConvertToLinear(diffuse);
	this->specular = ConvertToLinear(specular);

	this->emission = ConvertToLinear(emission);
...
Vector3 Material::ConvertToLinear(Vector3 original)
{
	//return Vector3(c_linear(original.x), c_linear(original.y), c_linear(original.z));
	return original;
}
